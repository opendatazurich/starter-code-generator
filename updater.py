...
def create_overview(data, header):
    ...
    baselink_r_renku = f"https://renkulab.io/p/{RENKU_NAMESPACE}/{RENKU_PROJECT_SLUG}/sessions/{RENKU_SESSION_ID_R}/"

    md_doc = []
    md_doc.append(header)
    md_doc.append(
        f"| Title (abbreviated to {TITLE_MAX_CHARS} chars) | Python Colab | Python Renku | Python GitHub | R Renku | R GitHub | SQL Workbench | File |\n"
    )
    md_doc.append("| :-- | :-- | :-- | :-- | :-- | :-- | :-- | :-- |\n")
    ...
    for idx in data.index:
        ...
        download_url = encode_url_sqlworkbench(data.loc[idx,PREFIX_RESOURCE_COLS+'url']) # encoding for duckdb
        ...
        if fileformat in ('csv', 'parquet'):  
            encoded_providerlink = encode_url_sqlworkbench(PROVIDER_LINK)
            sql_workbench_link = f"""[![SQL](https://img.shields.io/badge/SQL-grey?style=flat&logo=DuckDB&logoSize=auto&labelColor=grey&color=grey&link=https%3A%2F%2Fsql-workbench.com)](https://sql-workbench.com/#queries=v0,%20%20-This-is-autogenerated-code-to-analyze-(meta)-data-from-{PROVIDER},%20%20-All-information-refers-to-the-data-set-that-can-be-found-here%3A-{encoded_providerlink}dataset%2F{package_name},%20%20-get-metadata,CREATE-TABLE-metadata-AS-SELECT-*-FROM-read_json(%22{encoded_providerlink}api%2Faction%2Fpackage_show%3Fid%3D{package_name}%22)~,%20%20-use-this-to-display-metadata,%20%20SELECT-result.title%2C-result.timeRange%2C-result.notes%2C-result.sszBemerkungen-FROM-metadata~,%20%20-get-data,CREATE-TABLE-ogdszh-AS-SELECT-*-FROM-read_{fileformat}(%22https%3A%2F%2Fcors.sqlqry.run%2F%3Furl%3D{download_url}%22)~,%20%20-show-the-first-5-rows-of-the-data,SELECT-*-FROM-ogdszh-LIMIT-5~)"""
        else:
            sql_workbench_link = ''

        md_doc.append(
            f"| [{title_clean}]({ds_link}) | {py_colab_link} | {py_renku_link} | {py_gh_link} | {r_renku_link} | {r_gh_link} | {sql_workbench_link} | {resource_format} |\n"
        )
...
def encode_url_sqlworkbench(url):
    ...
    """
    url_quote = quote(url, safe='')
    url_quote_special = url_quote.replace("-","%20")

    return url_quote_special
